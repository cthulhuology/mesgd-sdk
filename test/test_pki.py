...
class TestPKI(unittest.TestCase):
    ...
    def test_sign_verify(self):
        ...
        self.assertTrue(PKI.verify(message, sig))
        self.assertFalse(PKI.verify('Wrong message', sig))
    ...
    def test_encrypt_decrypt(self):
        ...
        wrong_enc = PKI.encrypt('Different message')
        with self.assertRaises(Exception):
            PKI.decrypt(wrong_enc + 'invalid')

    def test_init_twice(self):
        # Should load existing keys without regenerating
        original_file_size = os.path.getsize(PKI.FILE)
        PKI.init()
        self.assertEqual(os.path.getsize(PKI.FILE), original_file_size)
